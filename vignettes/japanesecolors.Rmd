...
`dataviz_friendly` screens for the two things that decide it: whether the
colours survive colour-vision deficiency, and whether they are far enough apart
to read as different at a glance.
...
`min_delta_e` is the closest pair in normal vision and `min_delta_e_cvd` the
closest under the worst of simulated deuteranopia, protanopia and tritanopia,
both as CIEDE2000 distances. They ship with every palette, so you can set a
different bar than the default.
